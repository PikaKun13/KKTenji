...
3. **Windows 11 で「スマート アプリ コントロール」が有効な PC では、未署名アプリは
   ハードブロックされ実行できません**（「詳細情報」を押しても解説ページが開くだけ）。
   実行するには 設定 → プライバシーとセキュリティ → Windows セキュリティ →
   アプリとブラウザー コントロール → スマート アプリ コントロール を **オフ** にしてください
   （一度オフにすると Windows の再インストールまで再有効化できない点に注意）
4. 起動して **「サンプルを開く」** で動作を確認

インストール後は、エクスプローラーの右クリック「KKTenji で開く」（.pptx / .md / フォルダ）や、
ウィンドウへのドラッグ&ドロップでも deck を開けます。

**複数の deck を同時に開けます**: 右クリック「KKTenji で開く」や再起動のたびに新しい窓が開き、
窓ごとに別の deck を表示できます（コマンドバーの「新しい窓」/ Ctrl+N でも）。
1 つの窓で別の deck に切り替えたいときは「閉じる」（Ctrl+W）で Welcome に戻ります。
...
| Ctrl+W / Ctrl+N | deck を閉じて Welcome へ / 新しい窓を開く |
...
- インストーラは未署名（SmartScreen 警告が出ます。上記の手順で続行。
  スマート アプリ コントロール有効機ではオフにしない限り実行不可）
